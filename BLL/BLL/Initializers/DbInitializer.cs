        ...
        public void InitRealTimePositions()
        {
            ...
            for (int i = 0; i < cardPersons.Count; i++)
            {
                ...
                else//这部分随机初始位置
                {
                    ...
                    var tagposition = new LocationCardPosition() { CardId = cp.LocationCardId, Id = card.Code, X = x, Y = 2, Z = z, DateTime = dt, DateTimeStamp = TimeStamp, Power = 0, Number = i, Flag = "0:0:0:0:0", AreaId = 2, PersonId = cp.Id };
                    tagpositions.Add(tagposition);
                }
