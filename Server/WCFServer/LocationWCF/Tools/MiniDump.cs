    ...
    public static class MiniDump
    {
        ...
        public static Boolean TryDump(String dmpPath, MiniDumpType dmpType)
        {
            ...
            using (FileStream stream = new FileStream(dmpPath, FileMode.Create))
            {
                ...
                /*
                 这里要注意的一点是当前线程的ID，不能用当前线程的托管线程ID(即Thread.CurrentThread.ManagedThreadId)，
                 而是用当前线程的操作系统ID(即OSID)，否则用windbg加载dump文件时可能会出现一些错误信息：
                 ...
                 */
                //mei.ThreadId = Thread.CurrentThread.ManagedThreadId;//这个不能用
